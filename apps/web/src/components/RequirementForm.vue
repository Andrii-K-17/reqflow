...
import { Layers, AlignLeft, Flag, FileText, Quote, AlertTriangle, Loader2, X } from '@lucide/vue'
...
const typeOptions: SelectOption[] = [
  ...
  { value: 'USER', label: 'User requirement' },
  { value: 'FUNCTIONAL', label: 'Functional requirement' },
  { value: 'NONFUNCTIONAL', label: 'Non-functional requirement' },
  { value: 'SYSTEM', label: 'System requirement' },
]
    ...
    >
      ...
      <div class="space-y-5">
        <div>
          ...
          <AppSelect v-model="type" :options="typeOptions" :icon="Layers" />
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="flex items-end pb-3">
            <label
              class="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300 hover:cursor-pointer"
            >
              <button
                type="button"
                class="relative h-6 w-11 shrink-0 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-200/30 dark:focus:ring-blue-950/50"
                :class="
                  verifiable ? 'bg-sky-500 dark:bg-sky-600' : 'bg-slate-300 dark:bg-slate-700'
                "
                @click="verifiable = !verifiable"
              >
                <span
                  class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow-sm transition-transform"
                  :class="{ 'translate-x-5': verifiable }"
                />
              </button>
        ...
        <p
          ...
          class="flex items-start gap-1.5 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-700 dark:bg-amber-500/10 dark:text-amber-400"
        >
